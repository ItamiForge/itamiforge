...
impl PortInfo {
    ...
    pub fn smart_label(&self) -> String {
        ...
        match self.port {
            ...
            3000 | 3001..=3010 => return "Node/React".to_string(),
        ...
        if name.contains("node") { return "Node".to_string(); }
        if name.contains("python") { return "Python".to_string(); }
        if name.contains("docker") { return "Docker".to_string(); }
        if name.contains("chrome") { return "Chrome".to_string(); }
        if name.contains("code") { return "VS Code".to_string(); }
...
pub fn scan_range(range: &str) -> Result<()> {
    ...
    if !in_use.is_empty() && in_use.len() <= 20 {
        println!("\nIn use: {}", in_use.iter().map(|p| p.to_string()).collect::<Vec<_>>().join(", "));
    }
